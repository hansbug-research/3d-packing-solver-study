...
当前综合证据仍不是全套件完成：`12/32` benchmark 有实际执行，`32/32` benchmark 有状态记录，`549/608` cell 有证据，其中 `54` 个 cell 已执行 protocol-v3、`465` 个是 status-only，合计 `62,323` 条记录（legacy `2,078`，protocol-v3 `60,245`）。新增记录包括 B01/B02 的 `RELAXED_ALL_ROTATIONS` projection（`22,880` 条）、B01/B02 PackingSolver native certificate revalidation（`1,430` 条）、B07 BR0/BR8–15 的 projection（`30,600` 条）以及 4 条 B07 source-rotation exact calibration；projection 与原始姿态语义分轨，不能覆盖 native 结果。native revalidation 明确标为 `ARCHIVED_CERTIFICATE_REVALIDATION`，复用了已归档并独立重验的 certificate，不是一次新的 solver invocation；输入 hash、fork commit、binary hash 和原始验证行均保留。B07 projection 覆盖 900 个来源实例、八个 Python/Go/Rust projection 实现、两种排序和 1 s/10 s 预算，并在每条记录中固定来源 commit 与 CSV hash。新增 status-only 记录只表示来源/能力边界，不能视为求解运行，也不计入 executed cells。本轮约束 gauntlet 覆盖四个 PackingSolver 版本变体和 30 条实例记录；它补充了硬约束行为证据，不能替代其他库的全量 adapter。B05 来源仍未冻结，B08、B10–B11 和 B19+ 尚未形成全库共同适配器，B24–B32 也只完成局部专项；在这些门禁完成前，报告只宣称“已完成子集结果 + 覆盖计划”，不宣称 ALL-libs 全量完成。
