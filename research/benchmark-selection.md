...
建议按“先校准语义，再测几何质量，最后测工业组合和运维”的顺序执行。每一波都要求所有候选实现产生一条记录；不能运行的实现必须写 `NOT_SUPPORTED`、`ADAPTER_MISSING` 或 `SOURCE_INVALID`，这样分母保持不变。

| 波次 | Benchmark | 进入下一波的门禁 | 这波可以发布的结论 |
|---|---|---|---|
| **Wave 0：模型校准** | B06、B12-B18、B24-B26、B29 | 独立 validator、单位/姿态/数量和故障协议通过；exact 小例能闭合或给出明确 infeasible proof | 库是否正确理解输入和硬约束；这是资格门，不是质量排名 |
| **Wave 1：经典几何** | B01、B02、B04、B07 | 同一 source hash、同一预算、合法完整证书率可复核；B04 必须装完全部需求 | 正交 knapsack/bin-packing 的质量、旋转/排序敏感性和时间响应 |
| **Wave 2：目标与资源** | B03、B08-B11、B30 | 成本方向、有限 copies、开放维度和库存完整性由 exact-small 或下界校准；FULL 与 projection 分榜 | profit/成本/开放长度是否真的被优化，箱型分配是否正确 |
| **Wave 3：工业组合** | B19-B23、B31、B33-B34（候选） | source/license/canonical converter 完成；路线、层、堆叠、非规则字段逐条有 validator 证据 | 现实分布迁移和端到端合规率；不把删字段 projection 叫 FULL |
| **Wave 4：部署压力** | B27-B29、B32 | 多 seed、规模曲线、取消/崩溃恢复和在线增量语义可复现 | 延迟、RSS、稳定性、重排代价和托管风险；不评价未验证布局 |

发布策略上，Wave 0/1 是所有库的共同基线；Wave 2 只对能表达相应目标的库做 native/composed 比较；Wave 3/4 是按能力矩阵分层，不强行要求几何库承担路线、法规或在线 API。任何波次都不建议产生一个跨问题族总分，最终应输出每个问题族的合规门、质量表和未支持边界。

### 1.3 Q4RealBPP 与 3DBPPsi：建议新增但不替代经典套件
