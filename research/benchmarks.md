...
| **ESICUP VRPTW-CLP** | ESICUP `misc/vrptw_clp`，46 个实例文件；来源快照和文件哈希见 B21 audit | 路线/时间窗与三维装载的联合问题；当前快照 23 个文件有缺失高度标志、1 个文件客户行缺字段，保持 `SOURCE_INVALID/SOURCE_PENDING` | 来源修复后才能进入 FULL/projection 运行；不能丢弃行或猜补字段后沿用原 benchmark 名称 |
| **MPV official-generator-derived** | DIKU 官方 `3dbpp.c`/`test3dbpp.c`/`readme.3dbpp`；源码 SHA、参数和 150 个 canonical 输入见 [`b05-official-generator-derived.md`](b05-official-generator-derived.md) | 固定方向同型多箱；type 1/6/7/8/9，30/60/90 件，每组 10 个 replicate | 可复现地检查 MPV 论文生成分布、固定姿态和 exact baseline；它是派生轨道，不是论文原始 archive，官方代码声明仅限 research/academic use |
