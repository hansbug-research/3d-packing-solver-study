...
| 库/算法 | 6 轴向旋转 | 24 面语义姿态 | 直立/姿态子集 | 连续任意角 | 多箱装完 | 异构箱成本 | 有限 copies | 箱总重量 | 叠层/层高 | 最大堆叠数 | 最大上方重量 | 部分支撑/稳定 | 重心/轴荷 | 多站卸货 | 障碍物 | 最优性界 | Python 接入 | 核心语言 | 结果证书 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
...
| Skjolber | LAFF 几何强启发式 | Apache-2.0，Java sidecar | 基准 70 件 28 箱，几何通过；库内约 8 ms（本次快照） | 有独特障碍/controls 需求再引入 |
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| **Tauri 2 + React/TypeScript + Three.js** | ✅ | ✅ 子进程/sidecar | ✅ WebGL/Three.js；WebGPU 与 GPU 行为待三平台验证 | ✅ `InstancedMesh`（数据层 smoke） | ✅ | ⚠️ 壳通常小于 Electron，成品包体待测 | MIT/Apache 生态 | **首选桌面壳** |
| PySide6 + Qt Quick 3D | ✅ | ✅ 同进程/worker | ✅ Qt scene graph | ⚠️ 需自建批量策略 | ✅ | ⚠️ Qt 部署较重 | PySide6 LGPLv3/GPLv3/Commercial；Qt Quick 3D GPLv3/Commercial | Python-only 团队、原生控件优先 |
