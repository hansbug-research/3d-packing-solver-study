...
当前导入 2,078 条已有运行，并合并 B03、B07、约束 gauntlet 以及 B05 来源状态的 4,869 条 protocol-v3 记录，形成 13/32 个 benchmark、19 个实现/算法变体和 91/608 个计划单元的有证据记录；其中 45 个单元仍只有历史基线，B05 的 19 个单元是 `SOURCE_INCOMPLETE / NOT_RUN / SOURCE_PENDING` 状态记录而非实际执行。这不是综合 campaign 完成声明。其余单元在 `coverage.csv` 中继续显示 `SOURCE_PENDING`、`ADAPTER_MISSING`、`NOT_SUPPORTED` 或 `PLANNED`，不得把其中任何一种改写成已经实测。
